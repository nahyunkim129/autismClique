...
public class AutFirst {
	...
	public static double[][] extractGenes(String PathSelected, double[][] totalCoex, double threshold) throws FileNotFoundException {
 ...
 // below loop is to print double[][] matrix
		for (double[] subMAT : matrix) {
			for (int i = 0; i < subMAT.length; i++) {
				if(subMAT[i]<threshold) subMAT[i]=0.0;				
	...
	public static void main(String[] args) throws IOException {
		...
		String filteredGeneNamePath = "C:/Erica/Research/Autism/JavaAutism/Autism/exceptAutGenes.txt";
		...
		// coex =
		// extractGenes("C:/Erica/Research/Autism/JavaAutism/Autism/clique_I_test.txt",coex_total);
		String[] selectedAutGenes = new String[] { "Astn2", "Rims3" };

		GetNonAutGenes(selectedAutGenes, filteredGeneNamePath);
		coex = extractGenes(filteredGeneNamePath, coex_total,threshold);
		// Change printClique
		...
		AutFirst maxCliqueOpt = new AutFirst();
	...
	private static void GetNonAutGenes(String[] selectedAutGenes, String filename)
		...
		System.out.println(gname.size() + " total genes are loaded");
		all_gscan.close();

		System.out.println("Number of Autism : " + selectedAutGenes);

		PrintWriter writer = new PrintWriter(filename, "UTF-8");
		int temp=0;
		boolean found=false;
		for (String eachName : gname) {
			temp++;
			
			for (int i = 0; i < selectedAutGenes.length && !found ; i++) {				
				//System.out.print(eachName+" ? "+selectedAutGenes[i]+" : ");
				//System.out.println(eachName.toLowerCase().equals(selectedAutGenes[i].toLowerCase()));			
				if (eachName.toLowerCase().equals(selectedAutGenes[i].toLowerCase())) {
			...
			if(!found) writer.println(eachName);
			found=false;
		}
