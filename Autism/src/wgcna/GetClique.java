...
public class GetClique {
	...
	public ArrayList<Integer> FilterByAutism(ArrayList<Integer> Aut_index_al, ArrayList<ArrayList<Integer>> connected, int NumAutThrsh) throws IOException
	{				
		...
		for(int i=0; i<connected.size(); i++)
		{
			...
			int count=0;
			if(Aut_index_al.contains(i)) count++;
			for(int j=0; j<sub_connected.size(); j++)
			{											
				if(Aut_index_al.contains(sub_connected.get(j))) count++;
			}
	...
	public ArrayList<ArrayList<Integer>> FindConnected(List<double[]> Lists, double threshold) throws FileNotFoundException, UnsupportedEncodingException{
		...
		for(int k=0; k<Lists.size(); k++){
			...
			for(int i=k; i<List.length; i++){
				if(List[i]>threshold && i!=k) index.add(i);
			}
	...
	public void createInput(ArrayList<ArrayList<Integer>> connected, String filename, int NumNode) throws FileNotFoundException, UnsupportedEncodingException{
		
		PrintWriter writer = new PrintWriter(filename, "UTF-8");
		int edges=0;
		writer.println("1");
		writer.println(NumNode);		
		for(ArrayList<Integer> sub_connected : connected){
			edges = edges+sub_connected.size();
		}
		writer.println(edges); //3rd line
		for(int i=0; i<connected.size(); i++){
			ArrayList<Integer> sub = connected.get(i);
			for(int j=0; j<sub.size(); j++) writer.println(i+" "+sub.get(j)); //3rd line
		}
		writer.close();
	}
	
	private boolean NumAuts(int gene, ArrayList<Integer> sub_AL, int aut_threshold) {
		//Check if number of autism genes in this sub_AL greater than aut_threshold
		return true;
	}
	...
	public List<double[]> putInList(double[][] coex, double threshold) {
		...
		for (double[] matrix_row : coex) {
			...
			Arrays.fill(temp, 0);
			for (int j = 0; j < size; j++) { // column			
				if (matrix_row[j] > threshold) temp[j] = matrix_row[j];				
			}
			inList.add(temp);
		}
